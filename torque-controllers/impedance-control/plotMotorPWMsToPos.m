...
if 1
    ...
    for idx = 1:numel(pos)
        ...
        [Va(idx),Vb(idx),Vc(idx)] = deal(Va(idx)/10,Vb(idx)/10,Vc(idx)/10);
        [Vd(idx),Vq(idx)] = focEmul.computeIqId(Va(idx),Vb(idx),Vc(idx));
        
        [VaF(idx),VbF(idx),VcF(idx)] = focEmul.computePWMabc(Vdexp(idx),Vqexp(idx)*sqrt(3));
        [VaF(idx),VbF(idx),VcF(idx)] = deal(VaF(idx)/10,VbF(idx)/10,VcF(idx)/10);
        [VdF(idx),VqF(idx)] = focEmul.computeIqId(VaF(idx),VbF(idx),VcF(idx));
    end
    ...
    Plotter.plot2dDataNfittedModel(...
        figuresHandler,[System.toLatexInterpreterCompliant(joints{figIdx}) ' : 2FOC emulated Phase PWM voltages = f(Position). Actual VS Fixed'],['motor2focVabc2pos-Fig-2FOCemulFixed' num2str(figIdx)],...
        pos,Va,[],[],...
        'motor position (deg)','phase voltage PWM (dutycycle)',...
        'Va 2FOC',[]);
    
    Plotter.plot2dDataNfittedModel(...
        figuresHandler,[],['motor2focVabc2pos-Fig-2FOCemulFixed' num2str(figIdx)],...
        pos,Vc,[],[],...
        [],[],...
        'Vc 2FOC',[]);
    ...
    Plotter.plot2dDataNfittedModel(...
        figuresHandler,[System.toLatexInterpreterCompliant(joints{figIdx}) ' : expected d-q PWM = f(Position)'],['motor2focVdq2pos-Fig-2FOCemulFixed' num2str(figIdx)],...
        pos,Vdexp,[],[],...
        'motor position (deg)','voltage PWM (dutycycle)',...
        'Vd expected',[]);
    
    Plotter.plot2dDataNfittedModel(...
        figuresHandler,[],['motor2focVdq2pos-Fig-2FOCemulFixed' num2str(figIdx)],...
        pos,Vqexp,[],[],...
        [],[],...
        'Vq expected',[]);
    ...
    % Emulate 2FOC response to measured PWM Ta, Tb, Tc
    [Vdx,Vqx] = deal(zeros(size(pos)));
    for idx = 1:numel(pos)
        focEmul = FOCemulator();
        focEmul.updateState(round(pos(idx)*6));
        [Vdx(idx),Vqx(idx)] = focEmul.computeIqId(qV1(idx)/10,qV2(idx)/10,qV3(idx)/10);
    end
    
    Plotter.plot2dDataNfittedModel(...
        figuresHandler,[System.toLatexInterpreterCompliant(joints{figIdx}) ' : expected d-q PWM = f(Position)'],['motor2focVdq2pos-Fig-2FOCestim' num2str(figIdx)],...
        pos,Vdx,[],[],...
        'motor position (deg)','voltage PWM (dutycycle)',...
        'Vd estimated',[]);
